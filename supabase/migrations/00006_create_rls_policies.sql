...
--
-- PERFORMANCE NOTES:
-- 1. public.user_role() reads from app_metadata ONLY (that's where the custom token hook writes)
...
-- anon grant rationale: public.user_role() and public.user_id() are granted to anon
...
-- public.user_role() at all. The anon grant is harmless but worth documenting.
...
CREATE POLICY leads_consultant_create ON leads
  FOR INSERT WITH CHECK (public.user_role() = 'SALESPERSON');
...
CREATE POLICY project_state_history_insert ON project_state_history
  FOR INSERT WITH CHECK (public.user_role() IN ('ADMIN', 'MANAGER', 'SALESPERSON'));
...
CREATE POLICY design_templates_staff_read ON design_templates
  FOR SELECT USING (public.user_role() IN ('ADMIN', 'MANAGER', 'SALESPERSON', 'DESIGNER'));
...
CREATE POLICY design_elements_designer_write ON design_elements
  FOR ALL USING (
    public.user_role() = 'DESIGNER'
    AND template_id IN (SELECT public.designer_draft_template_ids())
  );
...
CREATE POLICY pricing_settings_consultant_read ON pricing_settings
  FOR SELECT USING (public.user_role() = 'SALESPERSON');
...
CREATE POLICY installation_reschedule_log_manager ON installation_reschedule_log
  FOR ALL USING (
    public.user_role() = 'MANAGER'
    AND project_id IN (SELECT public.manager_project_ids())
  );
...
CREATE POLICY audit_log_staff_insert ON audit_log
  FOR INSERT WITH CHECK (
    public.user_role() IN ('ADMIN', 'MANAGER', 'SALESPERSON', 'DESIGNER')
  );
