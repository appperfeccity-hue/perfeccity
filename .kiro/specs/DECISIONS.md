...
| AD-17 | Mobile encryption uses AES-256-GCM with key from `MOBILE_ENCRYPTION_KEY` env var (Edge Function only) | Key lives in Supabase secrets, never in code or config.toml. Encryption/decryption happens only in Edge Functions. DB-level access (Dashboard, direct SQL) cannot decrypt `mobile_encrypted`. Format: IV (12 bytes) ‖ ciphertext ‖ GCM auth tag. Fresh IV per encryption. | Admin viewing a lead's mobile MUST go through the Edge Function (`GET /leads/:id`), not raw SQL. DB exports will contain ciphertext only. Key rotation requires re-encrypting all existing rows (future operational task, not Sprint 1). | Yes |
