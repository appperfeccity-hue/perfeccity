...
| AD-13 | Rate limiting uses Postgres table (`login_attempts`), not in-memory or Redis | Edge Functions are stateless per-invocation — no in-memory counter survives between calls. Redis/Upstash would add an external dependency not in the MVP stack. Postgres table is self-cleaning (expired rows purged on each check), indexed on `(ip_address, attempted_at)`, tiny in practice. | Adds one DB write per failed login + one read per login attempt. Acceptable at MVP scale. | Yes |
